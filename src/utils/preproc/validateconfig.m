...
function Blade = checkblade(Blade, configFile, nAirfoils, DEF)
    ...
    for i = 1:numel(Blade)
        ...
        if numel(Blade) > 1
            ...
            if ~ismember(Blade(i).spinDir, [1, -1])
                error('ROTARE:validateconfig:incorrectSpinDir', ...
                      ['Expected Blade.spinDir to be either 1 (clockwise) or -1 ' ...
                       '(counter-clockwise).\nFound %d instead.'], Blade(i).spinDir);
            end
        end
